...
By default, the named filters aggregation returns an object. But in some sorting cases (bucket sort), the JSON doesn't
guarantee the order of elements in object. You can use the `sortable` parameter to specify the returned result is an
array of objects.

The value of this parameter can be as follows:

`false`::         Returns an object
`true`::          Returns the bucket in an array of objects

The following snippet shows a response where the `sortable` bucket is requested.
    ...
    "aggs": {
        "the_filter": {
            ...
            "aggs": {
                ...
                "sort_by_avg_price": { "bucket_sort": { "sort": { "avg_price": "asc" } } }
            }
        }
    }
}
